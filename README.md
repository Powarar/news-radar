...
6. добавляет плавный коэффициент свежести внутри трёхдневного окна:
   ...
   ```text
   personalized_score = relevance × 0.65 + freshness × 0.35
   ```

Предпочтения остаются главным сигналом, но свежая достаточно релевантная новость
может вытеснить старую с более высоким тематическим score. Если предпочтения ещё не заданы, применяется
...
Сейчас используются production-модели `openai/gpt-oss-20b` и
`openai/gpt-oss-120b`. Порядок можно менять через `GROQ_MODELS`. Один запуск
имеет общий лимит времени; при недоступном Groq классификация переходит на
локальный keyword fallback, а саммари может быть переобработано позднее.
...
Подробная карта классификации, embeddings, векторного поиска и RAG:
[docs/ai-rag.md](docs/ai-rag.md).
...
- улучшение semantic search/RAG: reranking, цитаты и кластеризация событий;
...
Эти направления не блокируют основные сценарии: сбор новостей, AI-обработку,
персональную выдачу, управление источниками и Telegram-уведомления.
