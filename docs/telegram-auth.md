...
## Два способа открыть сайт из Telegram

### Способ 1 — обычная URL-ссылка

Telegram открывает внешний браузер или встроенный браузер.
`window.Telegram` там **нет**. Это просто браузер, он не знает что ты пришёл из Telegram.

### Способ 2 — Mini App (Web App)
...
Это ключевая разница. В способе 1 нужно передавать токен вручную. В способе 2 Telegram делает это сам.

---

## Способ 1: Magic Link

Используется когда кнопка открывает браузер (не Mini App).
...
Пользователь пишет /start боту
        ↓
Бот → POST /api/v1/auth/telegram/magic-link
      заголовок X-Bot-Token: <токен бота>  ← защита от чужих запросов
      тело: {"telegram_id": "123456789"}
        ↓
Бэкенд находит или создаёт пользователя по telegram_id
Генерирует случайный код: secrets.token_hex(32) = 64 hex-символа
Кладёт в Redis: oauth_code:{код} → {access_token, refresh_token}
TTL = 300 секунд (5 минут), потом сам удаляется
        ↓
Бот получает {"code": "a3f9c2..."}
Бот отправляет кнопку:
  https://news.safonovpavel.space/tg-auth?code=a3f9c2...
        ↓
Пользователь нажимает → браузер открывает /tg-auth?code=a3f9c2...
Фронтенд читает ?code= из URL
Фронтенд → POST /api/v1/auth/exchange {"code": "a3f9c2..."}
        ↓
Бэкенд достаёт токены из Redis
Удаляет ключ (код одноразовый — больше не работает)
Возвращает {access_token, refresh_token}
        ↓
Фронтенд сохраняет токены в localStorage
Редирект на /feed — пользователь залогинен
```

### Почему одноразовый код, а не сразу токен в URL?

JWT-токен живёт 24 часа. Если его положить в URL:
- Telegram кэширует ссылки — ссылка может всплыть позже
- Токен попадает в историю браузера, логи сервера, реферер других сайтов

Одноразовый код живёт 5 минут и после первого использования исчезает из Redis.
Даже если кто-то перехватил ссылку — через 5 минут она мертва, а после первого перехода сразу.

---

## Способ 2: Mini App (основной)
